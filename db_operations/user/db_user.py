...
def get_user_info(user_ids):
    ...
    try:
        ...
        query = f"""
        SELECT u.id AS candidato_id, 
               u.nome, 
               u.avaliacao_curricular, 
               u.prova_de_conhecimentos, 
               u.nota_final, 
               GROUP_CONCAT(ub.contrato_id) AS contrato_ids,
               u.estado
        FROM Users u
        JOIN userbolsas ub ON u.id = ub.user_id
        WHERE u.id IN ({placeholders}) 
        GROUP BY u.id, u.nome, u.avaliacao_curricular, u.prova_de_conhecimentos, u.nota_final, u.estado
        ...
        return [{
            ...
            "nota_final": row[4],
            "contrato_ids": row[5],  # Updated to store aggregated contrato_ids
            "estado": row[6]
        } for row in results]
